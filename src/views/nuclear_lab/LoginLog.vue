  ...
  <div class="flex-auto flex flex-col bg-white rounded-3xl p-5 sm:p-8">
    ...
    <div class="flex-auto h-0">
      ...
      >
        ...
        <el-table-column prop="roleName" label="角色" :align="'center'" min-width="160" />
        <el-table-column prop="phoneNumber" label="手机号码" :align="'center'" min-width="160" />
        <el-table-column prop="createTime" label="登录时间" :align="'center'" min-width="160" />
        <el-table-column prop="remark" label="备注信息" :align="'center'" min-width="160" />
      </k-table>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getUserLoginLog(params) {
      ...
      try {
        const res = await nuclearLabApi.userLoginLog({
