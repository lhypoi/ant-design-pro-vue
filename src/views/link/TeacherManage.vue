        ...
        >
          <template v-if="!col.type" v-slot="scope">
            <div v-if="col.key === 'status'">
              <a-tag v-if="scope.row[col.key] === '0'" class="m-0">未认证</a-tag>
              <a-tag v-else-if="scope.row[col.key] === '1'" color="blue" class="m-0">待审核</a-tag>
...
export default {
  ...
  methods: {
    ...
    handleToDetail(row) {
      const routeUrl = this.$router.resolve({ name: 'TeacherDetail', query: { userId: row.userId } })
